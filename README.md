...
2. Система определяет проблему. Поиск по базе знаний выдаёт пять статей-кандидатов,
   ...
   ниже 0,75 система предлагает выбрать из трёх конкретных проблем кнопками
   или нажать «Другое» и описать подробнее.
...
специалиста; при отсутствии подходящей статьи ответ помечается как совет
ИИ-ассистента, а запрос попадает в список пробелов базы знаний; перезагрузка
страницы не прерывает диалог.
