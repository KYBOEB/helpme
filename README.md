...
### 4. Честная уверенность системы

Уверенность относится к **подобранной статье базы знаний**, а не к ответу вообще.
Если статья не подобрана, значения нет — в панели стоит прочерк. Это важное отличие:
показывать «90 %» рядом с ответом, которого нет в базе, значит вводить оператора
в заблуждение ровно там, где ему нужнее всего знать правду. Если уверенность ниже
порога 0,75 — система не гадает, а предлагает выбрать категорию кнопками.

### 5. Отсев нецелевых обращений

Помощник закрывает обращения, которые не относятся к технической поддержке:
бытовые и медицинские вопросы, оскорбления и провокации, бессмысленный набор
символов. Первое такое сообщение получает объяснение, чем помощник занимается —
человек мог просто ошибиться окном. Второе подряд закрывает обращение.

Главное здесь — **специалиста такие обращения не занимают**: они не попадают
в очередь и не портят метрику решённых. Отдельная защита работает без модели:
явная бессмыслица отсеивается локально, до обращения к внешнему API.

### 6. Карточка обращения, готовая к передаче
...
### 7. Живой специалист продолжает тот же диалог

Из панели оператор отвечает пользователю прямо в его чат — не почтой и не звонком.
Пользователь видит реплику в том же окне, помеченную как ответ специалиста, и может
ответить. Бот в эту переписку не вмешивается: он её только доставляет.
...
stateDiagram-v2
    [*] --> NEW
    NEW --> CLASSIFYING
    NEW --> NEW: вне тематики, первое предупреждение
    NEW --> RESOLVED: вне тематики повторно, закрыто без специалиста
    CLASSIFYING --> CLARIFYING: не хватает слотов
    CLASSIFYING --> SOLVING: уверенность ≥ 0.75, слоты собраны
    CLASSIFYING --> ESCALATED: статья не найдена
    ...
    ESCALATED --> ESCALATED: переписка со специалистом
    RESOLVED --> [*]
    ESCALATED --> [*]
```
...
| `ESCALATED` (взято оператором) | диалог ведёт человек: сообщения пользователя складываются в переписку, автомат не запускается |
...
|---|---|---|
| Полнота поиска по базе знаний | 40 из 40 | правильная статья попадает в топ-5 выдачи на всех тест-кейсах |
| Точность определения категории | 90% (36 из 40) | классификация обращения моделью на том же наборе |
| Доля обращений без уточняющих вопросов | 55% | у 22 карточек из 40 нет обязательных слотов — решение выдаётся сразу |
| Статей в базе знаний | 40 + добавленные оператором | 7 категорий каталога услуг, 3 карточки без готового решения |
| Проверок защиты API пройдено | 15 из 15 | `bash tests/attack_api.sh`, отчёт в [docs/SECURITY.md](docs/SECURITY.md) |
| Проверок поведения автомата | 29 из 29 | `python3 tests/test_new_features.py` — отсев нецелевых, уверенность, ответ специалиста |
| Нецелевых обращений отсеяно | считается системой | вкладка «Аналитика», поле `out_of_scope` в `GET /api/stats` |
| Стоимость обработки одного обращения | ≈ 0,24 ₽ | два вызова модели: дешёвая на классификацию, сильнее на ответ |
| Среднее число действий пользователя до решения | считается системой | видно на вкладке «Аналитика» в панели оператора |

### Про две первые строки

Их легко перепутать, поэтому мы их разделяем. **Полнота поиска** — попала ли нужная
статья в выдачу. **Точность категории** — угадала ли модель ярлык категории.

Все четыре промаха классификатора пришлись на одну границу каталога — «рабочее место»
против «оборудования»: «компьютер не включается», «синий экран», «сильно тормозит».
Это случаи, где и живой инженер ответит по-разному. При этом инструкцию пользователь
получает правильную: шаги выбираются найденной статьёй, а не ярлыком категории.
