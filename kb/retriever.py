...
#
# Порог поднят с 0,25 до 0,30 после роста базы до сотни карточек: на запросе
# «комп гудит и всё открывается вечность» лучшая близость была 0,252, то есть
# формально выше порога, а по сути — шум, и он вытеснил верную статью с пятого
# места из выдачи вовсе.
_MIN_COSINE = float(os.getenv("KB_MIN_COSINE", "0.30"))
