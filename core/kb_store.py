"""
База знаний = карточки из репозитория + карточки, добавленные оператором.
ВЛАДЕЛЕЦ: A.

Файлы из kb/articles ведёт участник C и правит через git. Карточки, созданные
оператором прямо из панели, кладутся в data/kb_custom — это примонтированный
том, поэтому они переживают пересборку контейнера.
